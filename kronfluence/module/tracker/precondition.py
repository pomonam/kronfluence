...
class PreconditionTracker(BaseTracker):
    ...
    def finalize_iteration(self) -> None:
        ...
        if self.module.factor_args.has_shared_parameters:
            ...
            self._compute_preconditioned_gradient(per_sample_gradient=self.cached_per_sample_gradient)
        self.clear_all_cache()

    def exist(self) -> bool:
        """Checks if preconditioned gradient is available."""
        return self.module.storage[PRECONDITIONED_GRADIENT_NAME] is not None
    ...
    def truncate(self, keep_size: int) -> None:
        ...
        if isinstance(self.module.storage[PRECONDITIONED_GRADIENT_NAME], list):
            ...
            self.module.storage[PRECONDITIONED_GRADIENT_NAME] = [
                self.module.storage[PRECONDITIONED_GRADIENT_NAME][0][:keep_size],
                self.module.storage[PRECONDITIONED_GRADIENT_NAME][1][:keep_size],
            ]
        else:
    ...
    def finalize_all_iterations(self) -> None:
        ...
        if self.module.storage[AGGREGATED_GRADIENT_NAME] is not None:
            ...
            self._compute_preconditioned_gradient(per_sample_gradient=self.module.storage[AGGREGATED_GRADIENT_NAME])
